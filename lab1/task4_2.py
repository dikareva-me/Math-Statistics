...
def task4_2():
    ...
    for name in distrs.distr_names:
        ...
        for j in range(len(sizes)):
            ...
            for k, scale in enumerate(bw):
                kde = gaussian_kde(arr, bw_method = 'silverman')
                h_n = kde.factor
                fig.suptitle(name + ", n = " + str(sizes[j]))
                ax[k].plot(interval, distrs.get_density_func(name, interval), color='red', alpha=0.5, label='density')
                ax[k].set_title(title[k])
                #sns.kdeplot(arr, ax = ax[k], bw = h_n * bw[k], label = 'kde', color = 'black')
                sns.kdeplot(data=arr, ax=ax[k], bw_method='silverman', bw_adjust=scale, label='kde')
                ax[k].set_xlabel('x')
                ax[k].set_ylabel('f(x)')
                ax[k].set_ylim([0, 1])
                if name == 'Poisson':
