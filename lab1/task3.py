...
def task3():
    ...
    for name in distrs.distr_names:
        ...
        line_props = dict(color = "black", alpha = 0.3)
        bbox_props = dict(color = "b", alpha = 0.9, lw = 2)
        flier_props = dict(marker = "o", markersize = 4)
        plt.boxplot((arr_20, arr_100), whiskerprops = line_props, patch_artist=True, boxprops = bbox_props, flierprops = flier_props, labels = ["n = 20", "n = 100"], vert = False)
        plt.ylabel("n")
        plt.xlabel("X")
        plt.title(name)
            ...
            for i in range(repeats):
                ...
                for k in range(0, sizes[j]):
                    if arr[k] > X[1] or arr[k] < X[0]:
                        count += 1
            count /= repeats
            rows.append(name + " n = $" + str(sizes[j]) + "$ & $" + str(np.around(count / sizes[j], decimals=2)) + "$")
