...
def task4_1():
    ...
    for name in distrs.distr_names:
        ...
        for j in range(len(sizes)):
            ...
            if name == "Poisson":
                ax[j].step(interval, [distrs.get_distr_func(name, x) for x in interval], color = 'red')
            else:
                ax[j].plot(interval, [distrs.get_distr_func(name, x) for x in interval], color = 'red')
            if name == "Poisson":
