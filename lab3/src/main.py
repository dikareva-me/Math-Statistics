...
def draw_data(data: np.ndarray, folder=".", ext="png"):
    ...
    for value in data:
        if value == cur_level:
            ...
            plt.plot([prev_count, cur_count], [cur_level, cur_level], 'darkblue')
            cur_count += 1
...
def draw_intervals(interval_data, folder=".", ext="png"):
    ...
    for i in range(len(interval_data)):
        plt.vlines(i, interval_data[i].low, interval_data[i].high, colors="darkblue", lw=1)
...
def draw_intervals_including_mode(interval_data, mode, folder=".", ext="png"):
    plt.clf()
    for i in range(len(interval_data)):
        color = "darkblue" if mode.inside(interval_data[i]) else "r"
...
def draw_mode_frequencies(interval_data, mu, k, folder=".", ext="png"):
    ...
    plt.plot(x, mu)
    plt.vlines([interval_data[k[0]].low, interval_data[k[-1]].high], [0, 0], [max(mu), max(mu)], color='g', ls='--')
...
def draw_oskorbin_optimization_results(interval_data, omega, beta, folder=".", ext="png"):
    ...
    plt.clf()
    for i in range(len(interval_data)):
        plt.vlines(i, new_data[i].low, new_data[i].high, colors="darkblue", lw=1)
    plt.plot([0, len(new_data)], [beta, beta], color='y')
